            ...
            <main className="flex-1 p-8 overflow-y-auto w-full">
                <header className="flex justify-between items-center mb-10">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-3xl font-bold gradient-text">{t('purchase_requests_title')}</h2>
                            <p className="text-gray-400 text-sm mt-1">{t('purchase_requests_subtitle')}</p>
                        </div>
                    </div>
                    ...
                    <div className="glass overflow-hidden border-border-custom">
                        <div className="p-6 border-b border-border-custom flex justify-between items-center bg-white/5">
                            <div className="flex items-center gap-3">
                                ...
                                <h3 className="text-xl font-bold">{t('active_requests')}</h3>
                            </div>
                        </div>
                            ...
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="border-b border-border-custom bg-white/2">
                                        <th className="p-6 text-xs font-bold uppercase text-gray-500">{t('request_title')}</th>
                                        <th className="p-6 text-xs font-bold uppercase text-gray-500">{t('priority')}</th>
                                        <th className="p-6 text-xs font-bold uppercase text-gray-500">{t('items')}</th>
                                ...
                                <tbody>
                                    {loading ? (
                                        <tr><td colSpan={5} className="p-10 text-center italic text-gray-500">{t('loading_requests')}</td></tr>
                                    ) : requests.length > 0 ? (
                                        requests.map((req: any) => (
                                            <tr key={req.id} className="border-b border-border-custom hover:bg-white/5 transition-colors group">
                                                <td className="p-6">
                                                    ...
                                                    <div className="text-[10px] text-gray-500">{req.description || t('no_description')}</div>
                                                </td>
                                                <td className="p-6">
                                                ...
                                                <td className="p-6 text-sm text-gray-400">
                                                    {req.items?.length || 0} {t('unique_items')}
                                                </td>
                                                <td className="p-6">
                                        ...
                                        <tr><td colSpan={5} className="p-20 text-center text-gray-500 italic">{t('no_purchase_requests_yet')}</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
                    ...
                    <div className="grid grid-cols-1 gap-6">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="flex flex-col gap-2">
                                <label className="text-xs font-bold text-gray-500 uppercase">{t('request_title')}</label>
                                <input
                                    type="text"
                            ...
                            <div className="flex flex-col gap-2">
                                <label className="text-xs font-bold text-gray-500 uppercase">{t('priority')}</label>
                                <select
                        ...
                        <div className="border-t border-border-custom pt-6">
                            <div className="flex justify-between items-center mb-4">
                                <h4 className="text-sm font-bold uppercase tracking-widest text-gray-500">{t('requested_items')}</h4>
                                <button
                                    type="button"
